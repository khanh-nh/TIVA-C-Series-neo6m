...
void SysTickIntHandler(void) {
    ...
    static char distance_str[16];  // Static buffer

    int int_distance = (int)lround(calculated_distance);  // Safely round the distance
    LCD1602Goto(0, 0);
    ...
    // Convert the integer to string manually
    int idx = 6;
    if (int_distance == 0) {
        distance_str[idx++] = '0';
    } else {
        int temp = int_distance;
        char reverse_str[10];
        int rev_idx = 0;
        while (temp > 0) {
            reverse_str[rev_idx++] = (temp % 10) + '0';
            temp /= 10;
        }
        while (rev_idx > 0) {
            distance_str[idx++] = reverse_str[--rev_idx];
        }
    }
    ...
    distance_str[idx++] = ' ';
    distance_str[idx++] = 'm';
    distance_str[idx] = '\0';  // Null-terminate the string
